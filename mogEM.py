...
def calcAngerHappy(x, y, anger, happy, mogLLL, LLLC):
    LL_anger = mogLLL(anger['prob'], anger['mu'], anger['var'], x)
    LL_anger = LL_anger + LLLC[0]
    LL_happy = mogLLL(happy['prob'], happy['mu'], happy['var'], x)
    LL_happy = LL_happy + LLLC[1]
    predict = (LL_anger < LL_happy)
    diff = float(np.sum(predict != y))
    size = y.shape[0]
